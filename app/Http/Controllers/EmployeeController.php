...
// dd($request->password);
    ...
    public function joiningletterPdf($id)
    {
        ...
        $joiningletter=JoiningLetter::where(['lang' =>   $currantLang,'created_by' =>  \Auth::user()->id])->first();
        $date=date('Y-m-d');
    ...
    public function ExpCertificatePdf($id)
    {
        ...
        $experience_certificate=ExperienceCertificate::where(['lang' =>   $currantLang,'created_by' =>  \Auth::user()->id])->first();
        $date=date('Y-m-d');
        $employees = Employee::find($id);
